...
## *Methods*\
We started by getting rid of variables that were notes, since they were mostly just all NAs. We renamed most variables to be more concise and usable. Then we transformed our variables into numeric and created some new variables:  
...
\
We then joined the two datasets by year and borough name. 
We found a second set of data about poverty rates in NYC, but it was located in a couple PDF files, so we input the data into a google sheets document, saved it as a CSV file, and brought it into our repository [^3] [^4]. 
